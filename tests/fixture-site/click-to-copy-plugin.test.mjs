// Click-to-copy as a JS plugin (alpine-plugin spike): the copy-button script
// moves from its own partial into the plugin registry as a theme-default
// entry, proving the registry serves plain imperative scripts and Alpine
// components alike. Legacy gates keep working: prism_syntax_highlighting
// supersedes it, disable_click2copy_chroma disables it.
...
test('click-to-copy ships as a deferred plugin by default', () => {
  ...
  assert.doesNotMatch(
    ...
    'the pre-plugin script path is gone',
  );
});
...
test('prism supersedes the copy-button plugin', () => {
  ...
  assert.doesNotMatch(
    html,
    /click-to-copy[^"]*\.js/,
    'no copy-button script tag alongside prism',
  );
});
